...
const DeliveryDeliverProduct = () => {
    ...
    const fetchSellerOrders = (searchTerm, setLoading, setOrders) => {
        ...
        let url = '/delivery-orders/PICKED_UP';
        if (searchTerm) {
...
// Call fetchSellerOrders when needed, passing the searchTerm, setLoading, and setOrders functions as arguments
    ...
    const handleSubmitDeliverOrder = (data) => {
        Swal.fire({
            title: "Are you sure?",
            text: "Do you want to deliver the products?",
            icon: "question",
            ...
            preConfirm: () => {
                return new Promise((resolve) => {
                    const requestData = {
                        delivery_user: data.delivery_user, order_id: data.id,
                        user_id: data.user
                    };
                    ...
                    axios.post('/send-mail/product/DELIVERED', JSON.stringify(requestData), {
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Token ${storeData.token}`,
                        },
                        ...
                        Swal.fire({
                            title: "OTP Sent to Customer's Email!",
                            ...
                            if (result.isConfirmed) {
                                ...
                                axios.post('/status-change/', JSON.stringify(requestData), {
                                    ...
                                    fetchSellerOrders(searchTerm, setLoading, setOrders);
                                    Swal.fire("Product has been delivered by you!", "", "success");
                                }).catch((err) => {
                    ...
                    <div class="row">
                        <div class="col-lg-8 offset-lg-2 text-center">
                            <div class="breadcrumb-text">
                                <p>Deliver Orders</p>
                                <h1>Pending Orders To Deliver</h1>
                            </div>
                        </div>
                    </div>
                </div>
...
export default DeliveryDeliverProduct;